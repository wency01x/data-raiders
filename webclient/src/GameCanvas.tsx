    ...
    const draw = () => {
      ...
      if (gs) {
        ...
        for (const e of gs.enemies || []) {
          ...
          // Bouncing arrow above targeted enemy
          if (e.id === tid) {
            const bounce = Math.sin(Date.now() / 250) * 5;
            const ax = e.x + TILE / 2;
            const ay = e.y - 14 + bounce;
            const aw = 12; // half-width of arrowhead
            const ah = 10; // height of arrowhead
            const as_ = 6; // arrow stem half-width
            const stemH = 8; // stem height

            ctx.beginPath();
            // Arrowhead (pointing down)
            ctx.moveTo(ax, ay + ah);           // tip
